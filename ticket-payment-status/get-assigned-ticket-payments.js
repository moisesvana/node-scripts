...
async function saveResults(items, fulfilledPayments) {
  ...
  // Save assigned tickets with fulfilled payments
  if (fulfilledPayments.length > 0) {
    const fileName = `assigned-tickets-fulfilled-${timestamp}.json`;
    const filePath = path.join(logsDir, fileName);

    const resultsData = {
      summary: {
        totalAssignedWithFulfilled: fulfilledPayments.length,
    ...
    try {
      ...
      console.log(
        `📁 Assigned tickets with fulfilled payments saved in: ${fileName}`
      );
      console.log(
        `📊 Total assigned tickets with fulfilled payments: ${fulfilledPayments.length}`
      );
    } catch (writeError) {
      console.error(`❌ Error saving results file:`, writeError.message);
    }
  } else {
    console.log("✅ No assigned tickets with fulfilled payments found");
  }

  // Save all assigned tickets (for reference)
  if (items.length > 0) {
    const allFileName = `assigned-tickets-all-${timestamp}.json`;
    ...
    try {
      ...
      console.log(`📁 All assigned tickets saved in: ${allFileName}`);
      console.log(`📊 Total assigned tickets: ${items.length}`);
    } catch (writeError) {
...
async function saveAllFailedItems(failedItems) {
  ...
  const timestamp = Date.now();
  const fileName = `failed-assigned-tickets-${timestamp}.json`;
  const filePath = path.join(logsDir, fileName);
  ...
  const allFailedData = {
    summary: {
      ...
      ticketsIndex: ticketsIndexName,
      paymentsIndex: paymentsIndexName,
      operation: "read assigned tickets and query stm_payment",
    },
...
async function init() {
  ...
  console.log("Starting to read assigned tickets from OpenSearch...");
    ...
    try {
      ...
      for (const item of items) {
        ...
        try {
          ...
          // Add to assigned tickets list
          assignedTickets.push(item);
  ...
  console.log(`Total assigned tickets: ${assignedTickets.length}`);
  console.log(`Assigned tickets with FULFILLED payments: ${fulfilledCount}`);
  console.log(`Failed items: ${failedCount}`);
  console.log("========================================");

  // Save all results
  await saveResults(assignedTickets, fulfilledPayments);
