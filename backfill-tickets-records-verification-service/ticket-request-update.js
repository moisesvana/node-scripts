...
async function saveFailedTicket(
  originalTicket,
  newTicketItem,
  error,
  ticketId,
...
async function saveAllFailedTickets(failedTickets) {
  ...
  const fileName = `all-failed-tickets-${timestamp}.json`;
...
const TYPE_VALUE = "TICKET_VERIFICATION";
...
async function init() {
  ...
  do {
    ...
    try {
      ...
      for (const ticket of data.Items) {
        ...
        console.log(`SK: ${ticket.sk}`);
        const loanRequestId = ticket.props.value;
        const ticketId = ticket.pk.split("|")[1];
        console.log(`Ticket ID: ${ticketId}`);
        console.log(`Loan Request ID: ${loanRequestId}`);
        const newTicketItem = ticket;
        // Execute transaction: delete old + create new
        const currentCreatedAt = ticket.created_at;
        const newCreatedAt = new Date(
          new Date(currentCreatedAt).getTime() + 1000
        ).toISOString();

        console.log(`Current Created At: ${currentCreatedAt}`);
        console.log(`New Created At: ${newCreatedAt}`);
        ...
        try {
          const updateParams = {
            ...
            UpdateExpression: "SET #created_at = :newCreatedAt",
            ExpressionAttributeNames: {
          ...
          console.log(
            `✅ Transacción exitosa: ${ticket.pk} → ${newTicketItem.pk}`
          );
        } catch (error) {
          console.error(
            `❌ Error en transacción para ${ticket.pk}:`,
            error.message
          );

          // Guardar ticket fallido en el folder fails
          await saveFailedTicket(
            ticket,
            newTicketItem,
            error,
            ticketId,
  ...
  console.log(`Total de tickets fallidos: ${failedCount}`);

  // Guardar todos los tickets fallidos en un solo archivo
  await saveAllFailedTickets(failedTickets);
}
